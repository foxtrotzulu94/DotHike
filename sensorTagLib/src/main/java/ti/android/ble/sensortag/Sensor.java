...
/**
 * This enum encapsulates the differences amongst the sensors. The differences include UUID values
 * and how to interpret the characteristic-containing-measurement.
 */
public enum Sensor {
	IR_TEMPERATURE(UUID_IRT_SERV, UUID_IRT_DATA, UUID_IRT_CONF, UUID_IRT_PERI, (byte) 30,
			(byte) 100) {
		@Override
		public Point3D convert(final byte[] value) {

			/*
			 * The IR Temperature sensor produces two measurements; Object ( AKA target or IR)
			 * Temperature, and Ambient ( AKA die ) temperature.
			 * 
			 * Both need some conversion, and Object temperature is dependent on Ambient
			 * temperature.
			 * 
			 * They are stored as [ObjLSB, ObjMSB, AmbLSB, AmbMSB] (4 bytes) Which means we need to
			 * shift the bytes around to get the correct values.
			 */
	...
	ACCELEROMETER(UUID_ACC_SERV, UUID_ACC_DATA, UUID_ACC_CONF, UUID_ACC_PERI, (byte) 10, (byte) 100) {
		@Override
		public Point3D convert(final byte[] value) {
			/*
			 ...
			 * (g = 9.81 m/s^2)
			 * 
			 * The z value is multiplied with -1 to coincide with how we have arbitrarily defined
			 * the positive y direction. (illustrated by the apps accelerometer image)
			 */
	...
	HUMIDITY(UUID_HUM_SERV, UUID_HUM_DATA, UUID_HUM_CONF, UUID_HUM_PERI, (byte) 10, (byte) 100) {
		@Override
		public Point3D convert(final byte[] value) {
	...
	MAGNETOMETER(UUID_MAG_SERV, UUID_MAG_DATA, UUID_MAG_CONF, UUID_MAG_PERI, (byte) 10, (byte) 200) {
		@Override
		public Point3D convert(final byte[] value) {
	...
	GYROSCOPE(UUID_GYR_SERV, UUID_GYR_DATA, UUID_GYR_CONF, UUID_GYR_PERI, (byte) 7, (byte) 10,
			(byte) 200) {
		@Override
		public Point3D convert(final byte[] value) {
	...
	BAROMETER(UUID_BAR_SERV, UUID_BAR_DATA, UUID_BAR_CONF, UUID_BAR_PERI, (byte) 10, (byte) 100) {
		@Override
		public Point3D convert(final byte[] value) {
			...
			O = c[5] * pow(2, 14) + c[6] * t_r / pow(2, 3) + ((c[7] * t_r / pow(2, 15)) * t_r)
					/ pow(2, 4);
	...
	SIMPLE_KEYS(UUID_KEY_SERV, UUID_KEY_DATA, null, null, (byte) 0, (byte) 0) {
		@Override
		public SimpleKeysStatus convertKeys(final byte[] value) {
			/*
			 * The key state is encoded into 1 unsigned byte. bit 0 designates the right key. bit 1
			 * designates the left key. bit 2 designates the side key.
			 */
	...
	/**
	 * Gyroscope, Magnetometer, Barometer, IR temperature all store 16 bit two's complement values
	 * in the awkward format LSB MSB, which cannot be directly parsed as getIntValue(FORMAT_SINT16,
	 * offset) because the bytes are stored in the "wrong" direction.
	 * 
	...
	private byte minPeriod, defaultPeriod;
	...
	/**
	 * Constructor called by the Gyroscope because he needs a different enable code.
	 */
	private Sensor(UUID service, UUID data, UUID config, UUID period, byte enableCode,
			byte minPeriod, byte defaultPeriod) {
	 ...
	 * */
	private Sensor(UUID service, UUID data, UUID config, UUID period, byte minPeriod,
			byte defaultPeriod) {
		this.service = service;
		this.data = data;
		this.config = config;
		...
		this.minPeriod = minPeriod;
		this.defaultPeriod = defaultPeriod;
		this.enableCode = ENABLE_SENSOR_CODE; // This is the sensor enable code for all sensors
												// except the gyroscope
	}

	/**
	 * @return the code which, when written to the configuration characteristic, turns on the
	 *         sensor.
	 * */
	...
	/** The default period value on SensorTag start-up, in tens of milliseconds. */
	public byte getDefaultPeriod() {
		return defaultPeriod;
	}

	/** The minimum period value for the sensor, in tens of milliseconds. */
	...
	public static final Sensor[] SENSOR_LIST = { IR_TEMPERATURE, ACCELEROMETER, MAGNETOMETER,
			GYROSCOPE, HUMIDITY, BAROMETER, SIMPLE_KEYS };
}
